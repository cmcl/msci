...
terminating session type for $\cdots$ and $\cdots$ respectively \todo{what are
  the interpretations for $\outend$ and $\inend$? Need a figure for GV
  sessions}. CP presents linear logical connectives with session type
interpretations inspired by
\citeauthor{Caires:2010:STI}~\cite{Caires:2010:STI} shown in Figure~\ref{??}
\todo{figure for CP types with session type
  interpretation}. \citeauthor{Wadler:2014} describes a continuation-passing
style (CPS) translation from GV session types to CP classical linear logic
propositions.  \todo{terms and interpretations for CP/GV?} The process
