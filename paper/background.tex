...
(i.e. session type) between parent and child \todo{what part of session type
  is being discussed here? spawning process or spawning operation?}.
...
connectives \todo{Do I need to elaborate on the translation? Yet another
  figure?}. CP provides linear logical connectives with session type
interpretations inspired by
\citeauthor{Caires:2010:STI}~\cite{Caires:2010:STI} shown in
Figure~\ref{fig:cptnts} along with the terms of CP. The process calculus CP is
slightly different to $\pi$DILL by \citeauthor{Caires:2010:STI}, apart from
being classical in nature, the constructs for server replication are modified:
CP provides separate weakening and contraction rules whereas these are encoded
in one rule for process reduction in $\pi$DILL. Additionally, rather than a
...
like GV. \citeauthor{Wadler:2014} describes a continuation-passing style (CPS)
translation from GV types and terms to CP classical linear logic propositions
and processes, respectively. As presented, GV does not support all features of
CP and thus the translation is one way (GV to
