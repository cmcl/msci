...
here. CP provided challenges for metatheory library since it differ in many
respects to lambda calculi. for example, in the lambda calculus one does not
work up to permutation of binders whereas in a process calculus these kinds of
structural equivalences are standard. we require additional assumptions for
environment ordering and more attention is paid to the opening of
subprocesses. metatheory did not support permutations and as a result of this
work some basic operations and custom tactics were developed. the custom
tactics can solve simple permutation goals via simple proof search. this slide
...
\begin{frame}[fragile]
\oldframetitle{Principal Cut Reduction ($\beta_{!?}$)}
\begin{tabular}{c}
\begin{coq}
Lemma reduce_spawn:
