...
Lemma abp_lossyBB_procR_typing :
...
Proof.
  ...
  Case "Htrans = TRNackA".
    ...
    SCase "+ err1!i; lossy(err1,err2)".
      (* err1!i; rest *)
      eapply TypPrefixOutput with (s:=SDual (SNack r k r' (token_of_bool i)))
          (rho:=TSingleton (token_of_bool (negb i)))
          (t:=SDual (SNack r k r' (token_of_bool i)));
        [apply trdual_w_mdual_involution; destruct i; assumption
          | left; contradict H_xi_nin; discriminate_w_list
          | constructor; [ctx_wf; discriminate_w_list | x_in_G]
          | constructor; [ctx_wf; discriminate_w_list | destruct i; x_in_G]
          | left; reflexivity
          | reflexivity
          | ].
      (* (lossy(err1, err2), i.e. New d *)
