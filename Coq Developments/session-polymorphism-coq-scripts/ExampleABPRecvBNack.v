...
Lemma abp_recvB_Nack_procR_typing :
...
Proof.
  ...
  (* err2?y; rest *)
  apply TypPrefixInput with (s:=SDual (SNack1 r k t (token_of_bool (negb i))))
  ...
  apply TypNew with (s:=SRecv i)
      (L:=y :: z :: xc :: xerr2 :: xout :: "recv_true" :: "recv_false" :: nil);
  ...
  eapply TypPrefixOutput with (s:=SDual (SRecv i))
      (rho:=TChannel (SDual (SNack r k t (token_of_bool (negb i)))))
      (t:=SDual (SRecv1 i (SNack r k t (token_of_bool (negb i))) t));
    [apply trdual_w_mdual_involution; econstructor; reflexivity
      | left; discriminate
      | constructor; [destruct i; ctx_wf; discriminate_w_list | x_in_G]
      | constructor; [destruct i; ctx_wf; discriminate_w_list | x_in_G]
      | left; reflexivity
  ...
  eapply TypPrefixOutput with
      (s:=SDual (SRecv1 i (SNack r k t (token_of_bool (negb i))) t))
