(** This file presents a cofinite-quantification style mechanization of System 
 Fsub without contexts using locally nameless representation. 
...
Proof.
  ...
  Simplify by eauto; inversion H; clear H.
    ...
    exists (I1' ++ (map_pred_remove_zero I2')); split; eauto.      
      rewrite list_remove_app; rewrite swap_remove_map_pred_remove_zero.
...
Proof.
  ...
  split.
    ...
    replace (emptyset:list ltvar) 
      with (emptyset ++ (map_pred_remove_zero I1)) by (rewrite Ha''; eauto);
        firstorder.
    destruct (lclosed_t_ltvar_split _ _ _ _ Hb) as [I2 [Ha' Ha'']].
    replace (emptyset:list ltvar) 
      with (emptyset ++ (map_pred_remove_zero I2)) by (rewrite Ha''; eauto); 
        firstorder.
Qed.
(* *************************************************************** *)
...
Proof.
  ...
  replace (emptyset:list ltvar) 
    with (emptyset ++ (map_pred_remove_zero I)) by (rewrite Hb; eauto);
    eauto.
  ...
  econstructor 5 with L'.
    ...
    intros; unfold L' in H5; Destruct notIn by eauto.
      apply (H ({O ~> X ^^ U4} U2)); Simplify by eauto.
  ...
  constructor 5 with L. 
    ...
    intros; apply IHn.
      ...
      eauto with subst_aux.
        eapply map_eq_nil with (f := pred); eauto.
Qed.
(* *************************************************************** *)
...
Proof.
  ...
  split.
    pattern (emptyset:list ltvar) at 1; 
      replace (emptyset:list ltvar) 
      with (emptyset ++ map_pred_remove_zero Ia) by (rewrite H5; eauto); eauto.
    pattern (emptyset:list ltvar) at 1; 
      replace (emptyset:list ltvar) 
        with (emptyset ++ map_pred_remove_zero Ib) by (rewrite H7; eauto); auto.
  destruct IHtyping.
