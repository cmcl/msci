(** This file contains auxiliary lemmas for a list that are used in Flinear.v.
...
Proof.
  ...
  clear H7 H2 H IHD1.
  change AtomSetImpl.elt with atom in x1, x0.
  subst.
  cbv beta zeta in *; unfold not, iff in |- *;
       fold (a `notin` union (dom D1) (union (singleton x) (dom D2))) in |- *;
       unfold not in *; intro H; case H9; clear H9.
  rename H into H9; autorewrite with set_simpl in *;
    unfold not, iff in |- *;
    fold (~ (a = x \/ x `in` dom D1 \/ x `in` dom D2)) in H4;
    unfold not, iff in * |-.
  intuition.
Qed.
