(** This file presents an exist-fresh style mechanization of System Fsub 
 without contexts using locally named representation. 
...
Proof.
  intros.
  rewrite <- subst_seq_ftvar_ltvar_t with (T := T) (X := X) (U := S) by eauto.
  rewrite <- subst_seq_ftvar_ltvar_t with (T := U) (X := X) (U := S) by eauto.
...
Proof.
  ...
  Simplify by eauto using lclosed_t_subst_ftvar.
...
Proof.
  ...
  assert (sub ({A ~> Z^^S1} U2) ({B ~> Z^^S1}S2)).
    eapply sub_ltvar_subst_rename_ftvar; eauto.
  assert (sub_transitivity_on ({A ~> Z^^S1} U2)).
    apply H; Simplify by idtac.
    rewrite size_t_nochange_ltvar; eauto with arith.
  assert (sub ({A0 ~> Z^^S1} T2) ({B ~> Z^^S1} S2)) by auto.
...
Proof.
  induction k.
  firstorder using lt_n_O.
  destruct 2; intros; Simplify by try congruence.
  destruct t; intros; Simplify by try discriminate; inversion H1; subst; eauto.
  destruct t; Simplify by try discriminate; inversion H3; subst; eauto.
    set (L := FV_ee ({a ::~> y ** U} t1)).
    destruct (pick_fresh L) as [x' Hfresh].
    unfold L in Hfresh.
