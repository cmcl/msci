...
While not related to session-based type systems \citeauthor{Gay:2001:FFP} provides a $\pi$-calculus framework mechanised in Isabelle/HOL. The aim is to build a mechanisaton of session-based type systems on top of the $\pi$-calculus framework. The use of de Bruijn indices for both bound and free variables causes issues with variable substitution often requiring permutation of typing environments (see \citeauthor{Aydemir:2008:EFM}~\cite{Aydemir:2008:EFM} for a discussion on binder representations) and lifting of free variables during substitution; we avoid this issue in our representation choice (\S \ref{sec:approach}).

%% Channel endpoints are represented as a polarity on a name. The polarities are not constrained to have the same type as each other but must be unique in the typing enviornment. 
...
your proposed approach is feasible, but identify any risks.
 ...
 %% Additionally, we make some simplifying assumptions about substitution for bound variables to avoid the lifting constructs defined by~\citeauthor{Gay:2001:FFP} in this work.
