...
synonyms for the type $\1$ which denotes the terminal session type. The client
and server process bodies are as follows:
\begin{gather*}
\sdef{ClientBody_c}
...
\DSubD{1}{*}{Mechanisation of Wadler's system}
\Deliverable{2}{Closure side-effecting extension}
\DSubD{2}{1}{Implementation of the GV operational semantics}
\DSubD{2}{2}{Implementation of the closure side-effecting extension}
...
\DTask{2}{1}{11th January}{7}{18th January}{-}
\DTask{2}{2}{19th January}{18}{6th Feburary}{D2.1}
%\DTask{3}{3}{
\end{tabular}
\end{center}
...
do this~\cite{Lindley:2014:SPS}. The task allocation time indicated in the
work plan table. \IDTsk{2}{1} is a theoretical investigation into the
operational semantics, deciding whether the system by
...
(D2.1). Lastly, \IDTsk{2}{3} is an allocation of time to the task of
implementing the extension permitting closures with side-effects. In my work
plan, I have assumed \IDTsk{2}{2} implemented an operational semantics that
eases this particular extension which justifies the task allocation time. Both
\IDTsk{2}{2} and \IDTsk{2}{3} may also include revisions to the theoretical
treatment started in \IDTsk{2}{1} as a consequence of initial design
oversights.
