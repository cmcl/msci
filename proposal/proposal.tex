...
While not related to session-based type systems
\citeauthor{Gay:2001:FFP} provides a $\pi$-calculus framework in Isabelle/HOL
had the aim of mechanising in mind for session type support with linear and non-linear type system design
name introduction in proof development from custome user-defined tactics are an issue. in coq one can specify the names directly and allow custom tactics to receive arguments for names. generally setting the hypotheses up correctly will provide suitable names automatically and it is only during destruction of some hypotheses that Coq may generate unsuitable names
...
I intend to mechanise the calculi described by \citeauthor{Wadler:2012}~\cite{Wadler:2012} using the Coq proof assistant.
