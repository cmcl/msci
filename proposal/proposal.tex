...
%% \begin{prooftree}
%% \AxiomC{$\Gamma , x \colon A \triangleright t \colon T \triangleleft \Gamma ' , x \colon B$}
%% \UnaryInfC{$\Gamma \triangleright t \colon (A \gg B) \rightarrow T \triangleleft \Gamma '$} 
%% \end{prooftree}
...
behaves according to the dual protocol. For example, considering $\SILLbranchLTwo$, and taking $A^\bot$ to represent the dual of a protocol $A$, the typed process is behaving according to the protocol $\BChoice{A^\bot}{B^\bot}$, selecting from alternatives (in this case, $\inrterm$) and then behaving according to the selected protocol (in this case, protocol $B^\bot$).
