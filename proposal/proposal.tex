...
\newcommand{\Deliverable}[2]
           {\multicolumn{2}{|c|}{\textbf{D{#1}: {#2}}} \\ \hline}
\newcommand{\DSubD}[3]{D{#1}.{#2} & #3 \\ \hline}
\newcommand{\DTask}[6]{D#1T#2 & #3 & #4 & #5 & #6 \\ \hline}
\newcommand{\IDTsk}[2]{Task D{#1}T{#2}}
...
The conclusion specifies a variable in (C) but a type in (CCO). Thus, in the
...
longer names $y$ in the conclusion: \input{cp-output}
...
The deliverables can be split into three main areas as indicated in the
following table:

\begin{tabularx}{\linewidth}{|c|X|}\hline
Deliverable \# &  Description \\ \hline
\Deliverable{1}{Mechanisation of Wadler's system}
D1 & Mechanisation of Wadler's system \\ \hline
\Deliverable{2}{Closure side-effecting extension}
\DSubD{2}{1}{Implementation of the GV operational semantics}
\DSubD{2}{2}{Implementation of the side-effecting closure extension}
\Deliverable{3}{Report write-up}
\DSubD{3}{1}{Description of approach, Wadler mechanisation and related work}
\DSubD{3}{2}
      {Description of GV operational semantics (w/o side-effecting closure)}
\DSubD{3}{3}{Description of side-effecting closure and final version}
\end{tabularx}

The current proposal will form part of D3.1 (related work, etc.). The
remaining parts of D3 will be written up as the project progresses, conducted
in tandem with D1 and D2. The estimated work plan is given in the following
table and sub-divides the above deliverable stages into tasks. The tasks
marking the completion of a deliverable are noted. The tasks appear in
chronological order.

\begin{center}
\begin{tabular}{|*{5}{c|}}\hline
\multicolumn{5}{|c|}{\textbf{Estimated Work Plan}}\\ \hline
Task & Start Date & Duration (days) & End Date & Deliverables \\ \hline
\DTask{1}{1}{8th December}{15}{23rd December}{-}
\DTask{3}{1}{20th December}{3}{23rd December}{-}
\DTask{1}{2}{24th December}{17}{10th January}{D1}
\DTask{3}{2}{7th January}{11}{18th January}{D3.1}
\DTask{2}{1}{11th January}{14}{25th January}{-}
\DTask{2}{2}{23th January}{21}{16th Feburary}{D2.1}
\DTask{3}{3}{13th Feburary}{10}{23rd Feburary}{D3.2}
\DTask{2}{3}{17th Feburary}{14}{3rd March}{D2.2}
\DTask{3}{4}{1st March}{14}{15th March}{D3.3}
\DTask{*}{*}{16th March}{39}{24th April}{-}
\end{tabular}
\end{center}

\IDTsk{1}{1} involves completing the GV and CP definitions including reduction
and commuting conversion rules for CP which I aim to complete during
December. This will require definition of infrastructure for handling binders
for both GV, CP channels and CP propositional variables, and inductive
definitions for well-typed terms, commuting conversions and CP principal cut
reductions. So far I have completed some of the definitions for well-typed
terms and CP process rules; the remaining rules require the infrastructure for
substitution and opening of binders. \IDTsk{3}{1} will consist of an initial
structuring and write up of the mechanisation effort of Wadler's system. It
will include a detailed description of the approach taken (which can, in part,
be formed from the current proposal) and the related work (again, partly
formed from the current document). Additionally, it will describe any issues
which have arisen, including changes made to any of the definitions as
presented by Wadler, including the changes already detailed here.

For D1, it remains to define the translation between GV and CP which I
envisage will require several rules on typing derivations; I plan to define
these as inductive datatype constructors but I shall take care to reuse any
suitable libraries that are available, e.g. a library for handling CPS
translations. For this step of the work (\IDTsk{1}{2}), I made a cautious
estimate on duration, accounting some slack time for any delays in preceding
stages of the development. \IDTsk{3}{2} will document the translation phase
and will also be used to document the initial approach for the extension (see
table for overlapping task allocation). \IDTsk{3}{2} represents delivery of
the first draft (D3.1) of the report describing the mechanisation of Wadler's
system but omitting the theoretical treatment for the proposed extension.
...
do this~\cite{Lindley:2014:SPS}. \IDTsk{2}{1} is a theoretical investigation
into the operational semantics, deciding whether the system by
\citeauthor{Lindley:2014:SPS} offers a suitable basis for the extension, and
investigating the details of the side-effecting closure extension (assuming an
appropriate base operational semantics for GV). \IDTsk{2}{2} is then the
implementation of the suitable operational semantics (without side-effecting
closure), culminating in the first part of the second deliverable
(D2.1). Following this implementation work, I plan to write up the operational
semantics in the report (\IDTsk{3}{3}) which constitutes the second draft of
the report deliverable (D3.2). Lastly, \IDTsk{2}{3} is an allocation of time
to the task of implementing the extension permitting closures with
side-effects which includes necessary changes to the typing judgements and
translation between GV and CP. In my work plan, I have assumed \IDTsk{2}{2}
implemented an operational semantics that eases this particular extension
which justifies the task allocation time. Both \IDTsk{2}{2} and \IDTsk{2}{3}
may also include revisions to the theoretical treatment started in
\IDTsk{2}{1} as a consequence of initial design oversights. \IDTsk{3}{4}
constitutes the final version of the report, including discussion of the
side-effecting closure extension.

There is a risk of being unable to perform the side-effecting extension as
sketched in \S \ref{sec:approach}. Additionally, there is considerable slack
time identified in the work plan, indicated by \IDTsk{*}{*}. I believe these
task allocation time estimates to be an accurate reflection of the work
involved. However, the feasibility study is necessarily limited in scope, so I
shall review and update the above plan throughout the project as I proceed to
the various milestones I have set out, and if required will alter the above
plans if necessary.
